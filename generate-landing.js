...
const [sessionsResp, availResp] = await Promise.all([
...
const eventName  = session.event?.name ?? session.name;
    ...
    <section aria-label="Información del partido" class="match-banner">
      ...
      <div class="match-card">
        <div class="match-card-top">
          <div>
            <div class="match-event-name">${eventName}</div>
            ${subtitle ? `<div class="match-subtitle">${subtitle}</div>` : ''}
          </div>
          ${onSale ? '<div class="match-on-sale">On sale</div>' : ''}
        </div>
        ...
        <div class="match-avail-row">
          ...
          <span class="match-avail-value">${availLabel}</span>
        </div>
      </div>
    </section>`;
...
const sectorOptions = sectors.map(s => `              <option value="${s.id}">${s.name}</option>`).join('\n');
